...
def get_help_md():
    # Construct a minimal dummy io and args so Commands() initialization
    # does not receive None and attempt to access attributes like
    # io.default_buffer on a None object.
    class _DummyBuffer:
        def __getattr__(self, name):
            # Return a no-op callable for any buffer method that may be invoked
            return lambda *args, **kwargs: None

    class _DummyIO:
        def __init__(self):
            self.default_buffer = _DummyBuffer()

        def __getattr__(self, name):
            # Return a no-op callable for any IO method that may be invoked
            return lambda *args, **kwargs: None

    class _DummyArgs:
        def __getattr__(self, name):
            # Return None for any argument attribute access
            return None

    md = Commands(_DummyIO(), _DummyArgs()).get_help_md()
    return md
