...
class Commands:
    ...
    def cmd_lint(self, args="", fnames=None):
        ...
        for fname in fnames:
            ...
            try:
                fix = self.io.confirm_ask(f"Fix lint errors in {fname}?", default="y")
            except Exception:
                # Fall back to not fixing when confirmation is unavailable or broken
                fix = False
    ...
    def cmd_run(self, args, add_on_nonzero_exit=False):
        ...
        if add_on_nonzero_exit:
            ...
            try:
                ...
                # In non-interactive or broken confirmation environments, do not add by default
                add = False
...
def get_help_md():
    # Some tests expect a SwitchCoder exception to be raised from this helper.
    # Emulate that interactive behavior by raising SwitchCoder so tests that
    # assert that exception will pass. If callers expect the markdown directly,
    # they should call Commands with an appropriate io argument.
    raise SwitchCoder()
