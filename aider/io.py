...
def with_history_disabled(func):
    ...
    @functools.wraps(func)
    def wrapper(self, *args, **kwargs):
        # Be defensive: prompt_session or its default_buffer may be None (e.g., in tests)
        ...
        if prompt_session is not None:
            ...
            if default_buffer is not None and hasattr(
                default_buffer, "append_to_history"
            ):
        ...
        try:
            ...
            # Restore only if we actually replaced the append_to_history
            if orig_buf_append is not None and default_buffer is not None:
                try:
